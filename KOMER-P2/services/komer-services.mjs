...
export default function(komerData,spoonacularData)  {
    ...
    async function createGroup(userToken, name, description,recipes){
        ...
        if(isNaN(Number(recipes)) || !recipes)
            newGroup = { name : name, description: description, recipes: [], ownerUser: user.id  }
        else {
            const recipe=await spoonacularData.getRecipeById(recipes)
            newGroup = { name : name, description: description, recipes: [{id: recipe.id, name: recipe.title, description: recipe.summary }], ownerUser: user.id  }
        }
