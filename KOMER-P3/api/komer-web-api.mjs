...
export default function(services) {
    ...
    async function getGroups(req, resp){
        return await services.getGroups(req.user)
    }
    ...
    async function getDetailsFromGroup(req, resp){
        return await services.getDetailsFromGroup(req.user, req.params.id)
    }
    ...
    async function createGroup(req, resp){
        resp.status(201)
        return await services.createGroup(req.user, req.body.name, req.body.description, req.body.recipes)
    }

    async function addRecipe(req, resp){
        resp.status(201)
        return await services.addRecipe(req.user, req.params.id, req.body.recipeId)
    }
